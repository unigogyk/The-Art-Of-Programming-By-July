...
- 固定a，求后面bc的排列：abc，acb，求好后，(将abc)a和b交换，得到bac
- 固定b，求后面ac的排列：bac，bca，求好后，(将abc)c放到第一位置，得到cba
...
public class AlgoTest {
	...
	public static char[] array = "abcd".toCharArray();
	
	public static void CalcAllPermutation(char[] array, int from, int to){
		if(to <= 1)
			return;
		if(from == to){
			System.out.println(array);	//当到达字符数组末尾时，输出该字符数组
		} else {
			for(int j = from; j <= to; j++){
				swap(array, j, from);
				CalcAllPermutation(array, from+1, to);
				//这两个swap()方法相互对应，第一个是交换两个字符，第二个是把这两个字符又给换回来。
				swap(array, j, from);	
			}
		}
	}
	...
	public static void main(String[] args) {
		CalcAllPermutation(array, 0, array.length-1);
	}
}
```
